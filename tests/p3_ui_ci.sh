...
set -uo pipefail
TMPDIR="${TMPDIR:-/tmp}"
HOST_PID_FILE="$TMPDIR/ion-ci-p3u.pid"

cleanup() {
    [ -f "$HOST_PID_FILE" ] && kill "$(cat "$HOST_PID_FILE")" 2>/dev/null || true
    rm -f "$HOST_PID_FILE" ~/.ion/host.sock /tmp/ion-ci-p3u-*.log
...
cleanup; sleep 0.5
"$ION_BIN" serve start > /tmp/ion-ci-p3u-host.log 2>&1 &
echo $! > "$HOST_PID_FILE"
...
# Create worker
SID=$($RPC --method create_session --params '{"session":"p3-ui"}' 2>/dev/null | \
