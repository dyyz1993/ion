...
项目级别的记忆插件。自动维护大纲索引，根据用户输入异步检索相关记忆，通过 `on_context` 钩子在每次发 LLM 前直接注入 custom entry，**不经过 follow_up**。
...
**Rust 扩展（AgentExtension trait）**，非 WASM 插件。原因：
- 需要 async 文件 I/O
...
| **整理记忆** | 定时任务（每 N 轮触发） | 插件后台自动 |

## 插件事件记录（Custom Entry）

框架只分**两类**，`customType` 由插件自己定，框架不预定义：
...
{"type":"custom","id":"xxx","timestamp":"...",
 "customType":"memory_searching",     ← 插件自定
 "data":{"query":"rust"}}
```

```json
{"type":"custom","id":"xxx","timestamp":"...",
 "customType":"memory_consolidate",   ← 插件自定
 "data":{"reviewed":12,"removed":2}}
```

### 类型 B：注入 LLM + 记录

同时写入 session 和注入 LLM 上下文。`customType` 插件自定，UI 据此展示样式。

```json
{"type":"custom","id":"xxx","timestamp":"...",
 "customType":"memory_injected",      ← 插件自定
 "data":{"source":"preferences","entries":["mem_1"]}}
```

**框架不管 `customType` 的具体值是啥**——插件自己控制。加新插件也不需要改框架。

## 记忆整理（ Consolidation ）

插件每 N 轮（默认 5 轮）自动触发一次整理。
...
整理是**纯插件级别行为**，与当前对话上下文无直接关系。整理结果 UI 可展示，LLM 不消费。
