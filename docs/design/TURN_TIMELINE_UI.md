...
对话历史有两种截然不同的使用模式，应作为**两个独立模块**实现，不要混在一起：
...
| 维度 | 说明 |
|------|------|
| **数据粒度** | 每个 turn 一行（用户输入 + 耗时 + 步数 + 最终回复） |
| **数据量** | **小**——每条 turn 几百字节，50 条 ≈ 几十 KB |
| **加载策略** | **一次性全拉**（`list_turns`，不分页） |
| **交互** | 默认全折叠，点击 ▼ 懒加载该轮详情（`get_turn_detail`） |
| **主接口** | `list_turns`（§2.1） |
| **辅助接口** | `get_turn_detail`（展开时按需调，§2.3） |
| **UI 呈现** | 折叠条列表：`用户输入 ▸ 已工作 2分7秒 · 6步` + `最终回复` |
| **适合** | 会话数 ≤ 几百，快速浏览"发生过什么" |

### 模块 B：完整细节模式（深挖型）

> "这次执行到底发生了什么"、"debug 某轮工具调用"

| 维度 | 说明 |
|------|------|
| **数据粒度** | 每一条 message（含 thinking / 工具参数 / 工具结果 / 文本） |
| **数据量** | **大**——单个 read 结果可能几 KB～几百 KB |
| **加载策略** | **分页 + 游标**（`get_messages` + `limit` + `after`） |
| **交互** | 直接平铺所有 message，滚动浏览；F5 用游标定位回之前位置 |
| **主接口** | `get_messages`（§3.1，支持 view/limit/after） |
| **辅助能力** | cursor 刷新定位（§4.3）、view 视点过滤（live/since_compaction/full） |
| **UI 呈现** | 时间线流：`user → thinking → tool_call(read, args) → tool_result → text → ...` |
| **适合** | 单次执行深入分析、排查问题、看工具参数 |
...
模块 A（概览）                     模块 B（细节）
┌────────────────────┐            ┌────────────────────────┐
│ list_turns 一次拉完 │            │ get_messages 分页拉     │
│                    │            │                        │
│ ▸ Turn 0 · 2分7秒  │──点击──►   │ user: 帮我分析架构      │
│ ▸ Turn 1 · 3秒     │            │ thinking: 先读文件…     │
│ ▸ Turn 2 · 15秒    │            │ tool: read Cargo.toml   │
│                    │            │   → name=ion version=..│
│ （点击某 turn 展开  │            │ tool: grep src/agent/   │
│  也可进入细节视图） │            │   → 128 matches        │
└────────────────────┘            └────────────────────────┘
```

**关键决策**：
- 模块 A **不需要分页**（数据小，一次拉完体验更好，不用等下一页）
- 模块 B **必须分页 + 游标定位**（数据大，不分页会卡；F5 刷新需要回到之前位置）
- 两个模块可以**在同一页面并存**（Tab 切换或上下分区），但数据流独立
...
|------|------|
| §1 UI 元素 | 两模块共用 |
| §2 数据裁剪（list_turns + get_turn_detail） | **模块 A** |
| §3 接口清单 | 两模块（list_turns 属 A，get_messages 属 B） |
| §4 多用户同屏 | 两模块共用（Push 机制一样） |
| §5 entry 解析 | **模块 B**（get_messages 返回的 entries） |
| §6 设计原则 | 两模块共用 |
