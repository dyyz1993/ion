...
> **状态：已完成** — 三种 CLI 执行场景全部落地。`ion serve` / `ion --host` / 场景 2 编排 / 递归 idle / Team 编排 / Workflow Engine 全部可用。
>
> **目标：** ~~三种 CLI 执行场景全部落地，所有验证用例通过。~~ ✅ 已达成。

---


> **注意：以下 checklist 大部分已过时（被后续实现覆盖）。**
> 保留仅供历史参考。当前状态请看 AGENTS.md 的"已完成"段和测试统计表。
> 141 个 `- [ ]` 项中，绝大多数已经在后续开发中完成（只是没回来打勾）。
...
- [x] **1.1** 新增 `ion serve` 子命令（内部调用 `cmd_manager_start`）
  - 位置：`src/bin/ion.rs` 的 `Commands` enum + main 分发
- [x] **1.2** 新增 `ion serve stop`（向 host socket 发 shutdown RPC）
- [x] **1.3** 新增 `ion serve status`（读 PID 文件 + `kill(pid, 0)` 检测存活）
- [x] **1.4** 删除 `Commands::Manager` + `ManagerAction`（彻底移除，不留隐藏别名）
- [x] **1.5** socket 路径改名 `manager_socket_path()` → 返回 `~/.ion/host.sock`
  - 位置：`src/paths.rs`
  - 加自动迁移逻辑：如果新路径不存在但旧路径存在，软链或复制
- [x] **1.6** PID 路径改名 `manager_pid_path()` → 返回 `~/.ion/host.pid`
- [x] **1.7** 更新 AGENTS.md 架构部分所有 `manager start` → `serve`
...
- [ ] `ion manager start` 不再存在（用户用 `ion serve start`）
...
- [x] **2.1** 给 `Cli` 结构体加 `--host` flag（`#[arg(long)] host: bool`）
- [x] **2.2** 实现 `cmd_host()` 函数
  - 启动临时 `WorkerRegistry`
  ...
  - 等全部 idle（递归检测）→ 清理退出
  - 超时兜底：30 分钟
- [x] **2.3** ~~删掉 `cmd_team()` 和 `ion team` 子命令~~ ✅ 已删除（被 --host --agent coordinator 替代）
- [x] **2.4** 在 main 分发中：`cli.host == true` 时走 `cmd_host()`，否则走 `cmd_run()`
