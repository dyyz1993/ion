...
> **状态：V1 已实现** — 快照折叠扩展 + grep 追踪 + 查询接口全部实现,16 单元测试 + 3 E2E 测试通过。V2(会话级主题漂移折叠)暂不开发 — V1 已满足当前需求。
...
### 6.1 追踪范围:read + grep,不追 bash/find

详见 [§8 对标分析](#8-对标分析pi-现状)。三种工具的可追踪性:

| 工具 | 可追踪性 | 原因 | 状态 |
|------|---------|------|------|
| `read` | ✅ 高(参数即路径) | `arguments.file_path` 精确 | ✅ 已追踪 |
| `grep` | ✅ 高(解析输出) | `parse_grep_paths()` 从 `路径:行号:` 前缀提取 | ✅ 已追踪 |
...
| # | 待办 | 优先级 | 说明 |
|---|------|--------|------|
| 1 | **V2: 会话级折叠(主题漂移检测)** | 低 | 距离衰减起步 → embedding 相似度(V2.1)→ LLM 判断(V2.2)。详见下方 |
| 2 | ~~**read 工具加 offset/limit**~~ | ✅ 已完成 | tool 层切片,加 `cat -n` 行号输出(`tool.rs` format_lines) |
| 3 | ~~**追 grep(解析输出)**~~ | ✅ 已完成 | `parse_grep_paths()` 解析 ripgrep 输出,grep 已从 untracked_sources 移出 |
| 4 | ~~**Compaction 对齐 pi**~~ | ✅ 已完成 | 溢出恢复(A2) / 增量摘要(A5) / split-turn(A7) 全部实现 |
| 5 | ~~**工具输出截断**~~ | ✅ 已完成 | `truncate_tool_output()` 超 2000 行/50KB 截头尾 |
