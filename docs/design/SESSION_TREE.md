...
#### 0.2.1 消息 + 代码联动回滚（`--restore-code`）

回滚时可选"连代码一起恢复"——配合 [File Snapshot](./FILE_SNAPSHOT.md) 系统：

```bash
# ① 只回滚消息（代码不动）
ion --resume <sid> --rollback msg_005

# ② 回滚消息 + 恢复代码（磁盘文件恢复到 Turn 5 时状态）
ion --resume <sid> --rollback msg_005 --restore-code
```

`--restore-code` 的执行顺序：
1. `restore_code_to_turn(target_turn)` — 先恢复磁盘文件（File Snapshot 负责）
2. `make_rollback(target_entry_id)` — 再追加 leaf_pointer（Session Tree 负责）
3. 记录 `restore_point` 快照（可用于"撤销恢复"）

**注意**：`--restore-code` 要求 file-snapshot 扩展已启用。未启用时回退为纯消息回滚 + 警告。
详细 case 矩阵见 [FILE_SNAPSHOT.md §18](./FILE_SNAPSHOT.md#18-消息代码联动回滚--完整-case-矩阵)。
