...
>
> **前置**：已安装 `mcp-server-everything`（`npm install -g @modelcontextprotocol/server-everything`）
...
# 3. 另一个终端：查入口 Worker 的 MCP 状态
ENTRY_SID=$(ion rpc --method list_sessions 2>&1 | grep -o 'sess_[a-f0-9]*' | head -1)
...
```json
{
  "type": "response", "command": "get_mcp_servers", "success": true,
  "data": [{
    "name": "everything", "transport": "stdio", "status": "connected",
    "disabled": false, "tools": [
      {"full_name": "mcp__everything__echo", "original_name": "echo", "description": "..."}
    ], "error": null
  }]
}
```
...
**验证点：**
- ✅ `status` = `"connected"`（入口 Worker 启动时 connect_all 成功）
- ✅ `tools` 非空（工具发现成功）
- ✅ 只 spawn 了 1 个 `mcp-server-everything` 进程（`pgrep -f mcp-server-everything | wc -l` = 1）

#### F2 子 Worker 共享 MCP 连接（不自己 spawn）

```bash
# 1. 前置：F1 已完成（入口 Worker 已连接 MCP）
# 2. 入口 Worker 调 spawn_worker 创建 developer（通过 LLM 或 RPC 触发）
#    这里用 FauxProvider 模拟 LLM 调 spawn_worker：
ION_FAUX_SCRIPT=/tmp/faux_spawn.json ion --host "spawn developer"
# faux_script 内容：[{"text":"我来 spawn developer","tool_calls":[{"name":"spawn_worker","arguments":{"agent":"developer","task":"测试 MCP 共享"}}]}]

# 3. 查所有 Worker
ion rpc --method list_workers

# 4. 查 developer Worker 的 MCP 状态（关键：它应该看到 stdio server！）
DEV_SID=$(ion rpc --method list_sessions 2>&1 | grep -o 'sess_[a-f0-9]*' | tail -1)
ion rpc --session "$DEV_SID" --method get_mcp_servers
```

**预期响应（developer Worker）：**
```json
{
  "data": [{
    "name": "everything", "transport": "stdio", "status": "connected",
    "tools": [{"full_name": "mcp__everything__echo", ...}]
  }]
}
```

**验证点：**
- ✅ developer Worker 看到 **stdio server**（方案 A/B 下看不到，方案 C 下共享）
- ✅ 仍然只有 1 个 `mcp-server-everything` 进程（子 Worker 不自己 spawn）
- ✅ `status` = `"connected"`（共享入口 Worker 的连接状态）

#### F3 子 Worker 调 MCP 工具

```bash
# 1. 前置：F2 已完成（developer Worker 已创建）
# 2. developer Worker 的 LLM 调用 stdio MCP 工具
ion rpc --session "$DEV_SID" --method call_tool \
  --params '{"tool":"mcp__everything__echo","args":{"message":"from developer"}}'
```

**预期响应：**
```json
{
  "type": "response", "command": "call_tool", "success": true,
  "data": {"tool": "mcp__everything__echo", "output": "Echo: from developer"}
}
```

**验证点：**
- ✅ developer Worker 能调 **stdio** MCP 工具（方案 A/B 下做不到）
- ✅ 返回结果正确（`"Echo: from developer"`）
- ✅ 调用走共享池代理（developer Worker 没有直连 server 进程）
...
# 1. 准备 agent.md（项目级）
mkdir -p .ion/agents
cat > .ion/agents/restricted-dev.md << 'EOF'
---
name: restricted-dev
disallowed_tools:
  - mcp__everything__echo
---
你是受限的 developer，不能调 echo 工具。
EOF

# 2. 启动 host，用 restricted-dev agent
ion --host "测试"   # 入口 Worker agent=restricted-dev

# 3. 查工具列表
ENTRY_SID=$(ion rpc --method list_sessions | grep -o 'sess_[a-f0-9]*' | head -1)
ion rpc --session "$ENTRY_SID" --method get_active_tools
```

**预期响应（get_active_tools 片段）：**
```json
{
  "data": {
    "tools": ["read", "write", "bash", "mcp__everything__get-sum", ...],
    "count": 42
  }
}
```
...
**验证点：**
- ✅ 工具列表**不含** `mcp__everything__echo`（被 agent.md `disallowed_tools` 过滤）
- ✅ 其他 MCP 工具仍在（`mcp__everything__get-sum` 等）
- ✅ `call_tool mcp__everything__echo` 返回 `tool not found`

#### F5 场景 2 退出时 MCP 连接关闭

```bash
# 1. 启动场景 2
ion --host "做完了" &  HOST_PID=$!
sleep 10

# 2. 确认 MCP server 进程在运行
pgrep -f mcp-server-everything   # 有进程

# 3. 等 host 退出（任务完成 → 递归 idle → 自动关）
wait $HOST_PID

# 4. 验证 MCP server 进程已清理
pgrep -f mcp-server-everything   # 无输出（进程已终止）
```

**验证点：**
- ✅ host 退出后，`pgrep` 找不到 `mcp-server-everything`（进程已清理）
- ✅ 无残留的 MCP server 进程（僵尸进程检查）
...
# 2. 创建 session 1
SID1=$(ion rpc --method create_session --params '{"agent":"build"}' | grep -o 'sess_[a-f0-9]*')
sleep 10  # 等 MCP 连接

# 3. session 1 能看到 MCP
ion rpc --session "$SID1" --method get_mcp_servers  # status: connected

# 4. kill session 1 的 Worker（模拟 session 销毁）
ion rpc --method kill --params '{"worker_id":"..."}'

# 5. 创建 session 2
SID2=$(ion rpc --method create_session --params '{"agent":"build"}' | grep -o 'sess_[a-f0-9]*')
sleep 5

# 6. session 2 也能看到 MCP（连接保持）
ion rpc --session "$SID2" --method get_mcp_servers  # status: connected
```

**验证点：**
- ✅ session 1 销毁后，session 2 仍看到 `status: connected`
- ✅ MCP server 进程一直存活（没有因为 session 1 销毁而终止）
- ✅ 场景 3 下 MCP 连接是**常驻**的（跟 host 走，不跟 session 走）

#### F7 多 Worker 并发调用同一 MCP 工具

```bash
# 1. 启动场景 3，创建两个 session
ion serve &; sleep 5
SID1=$(ion rpc --method create_session --params '{"agent":"build"}' | grep -o 'sess_[a-f0-9]*')
SID2=$(ion rpc --method create_session --params '{"agent":"build"}' | grep -o 'sess_[a-f0-9]*')
sleep 10  # 等 MCP 连接
...
# 2. 两个终端同时调（模拟并发）
# Terminal 1:
ion rpc --session "$SID1" --method call_tool \
  --params '{"tool":"mcp__everything__echo","args":{"message":"from-1"}}'

# Terminal 2（同时）:
ion rpc --session "$SID2" --method call_tool \
  --params '{"tool":"mcp__everything__echo","args":{"message":"from-2"}}'
```

**预期响应（各自独立返回）：**
```json
// Terminal 1
{"data": {"output": "Echo: from-1"}}

// Terminal 2
{"data": {"output": "Echo: from-2"}}
```

**验证点：**
- ✅ 两个 Worker 并发调用同一 MCP 工具，**不冲突**
- ✅ 各自返回正确结果（`from-1` 和 `from-2`，不串号）
- ✅ rmcp Peer 的并发调用正常（多 producer channel）
...
# 2. 直接执行（场景 1，不经过 host）
ION_FAUX_REPLY="MCP 工具测试" ion "用 mcp__everything__echo 工具回复 hello"

# 3. 或者用 FauxProvider 模拟 LLM 调工具
ION_FAUX_SCRIPT=/tmp/faux_mcp.json ion "echo hello"
# faux_script: [{"tool_calls":[{"name":"mcp__everything__echo","arguments":{"message":"hello"}}]}]
```

**预期输出：**
```
[tool] mcp__everything__echo({"message":"hello"}) → "Echo: hello"
```

**验证点：**
- ✅ 场景 1 的 Agent 能看到 `mcp__everything__echo` 工具（get_active_tools 含它）
- ✅ LLM 调用 MCP 工具成功，返回 `"Echo: hello"`
- ✅ 执行完毕后 MCP server 进程退出（场景 1 跑完即退）

#### G2 场景 1 没配 MCP 零开销

```bash
# 1. config.json 不含 mcp_servers
...
**验证点：**
- ✅ 正常执行，无报错
- ✅ 没有 MCP 相关日志（`is_empty()` 跳过初始化）
- ✅ 没有 spawn 任何 MCP server 进程
...
cat > ~/.ion/config.json << 'EOF'
{
  "mcp_servers": {"everything": {"command": "mcp-server-everything"}}
}
EOF

# 2. 准备全局权限规则（禁用 echo 工具）
...
# 4. 尝试调用被禁用的工具
ion rpc --session "$SID" --method call_tool \
  --params '{"tool":"mcp__everything__echo","args":{"message":"blocked?"}}'
```

**预期响应：**
```json
{
  "type": "response", "command": "call_tool", "success": false,
  "error": "call_tool mcp__everything__echo: Tool call failed: permission denied"
}
```

**验证点：**
- ✅ 调用被 **Deny**（permission 扩展拦截，不执行）
- ✅ 错误信息含 `permission denied`
- ✅ MCP server 进程仍在运行（权限拒绝不影响连接）

#### H2 项目级规则覆盖全局

```bash
# 1. 前置：H1 的全局规则存在（Deny mcp__everything__echo）
# 2. 项目级 settings.json 覆盖为 Allow
cat > .ion/settings.json << 'EOF'
{
  "permissions": {
    "rules": [
      {
        "id": "perm_mcp_echo_allow",
        "provider": "user",
        "subject": "*",
        "pattern": "mcp__everything__echo",
        "decision": "Allow",
...
ion rpc --session "$SID" --method extension_rpc \
  --params '{"extension":"permission","method":"reload"}'

# 4. 再次调用（这次应该成功）
ion rpc --session "$SID" --method call_tool \
  --params '{"tool":"mcp__everything__echo","args":{"message":"allowed!"}}'
```
...
# 3. 调不同工具都应被拒
ion rpc --session "$SID" --method call_tool \
  --params '{"tool":"mcp__everything__echo","args":{"message":"x"}}'
# → permission denied

ion rpc --session "$SID" --method call_tool \
  --params '{"tool":"mcp__everything__get-sum","args":{"a":1,"b":2}}'
# → permission denied
```

**验证点：**
- ✅ 所有 `mcp__everything__*` 工具被禁用（echo / get-sum 都 Deny）
- ✅ 非 MCP 工具不受影响（`read` / `write` / `bash` 正常）
