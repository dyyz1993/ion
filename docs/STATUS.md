...
- **lib 984/0**；branch_tree_ci 13/13；ion-webui e2e T1-T20 20/20 + 视觉 16 态截图
- 真实 LLM 冒烟全链 ✓（glm-5.3-flash：流式/工具卡 v2/13s 正确作答，零 JS 错）
- 当日修复：worker 主循环新命令名别名（刷新空白终因）· create_session 幂等守卫（31ms 双开）
  · 复活 cwd 兜底（spawn ENOENT）· 循环检测累计口径 · FileIndex leaf 透传
  · 记忆库污染治理（四层堵源）· 会话存储大扫除（1889 孤儿归档，317→69 可见）
