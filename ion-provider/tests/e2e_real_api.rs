...
fn anthropic_conf() -> Option<(String, String, String)> {
    if std::env::var("ION_E2E_ANTHROPIC").is_err() {
        return None;
    }
    let base = std::env::var("ION_ANTHROPIC_BASE_URL").unwrap_or_else(|_| {
        "https://p.19930810.xyz:8443/k/glm/https://api.z.ai/api/anthropic".into()
    });
    let key = std::env::var("ION_ANTHROPIC_API_KEY")
        .unwrap_or_else(|_| std::env::var("ION_API_KEY").unwrap_or_default());
    let model = std::env::var("ION_ANTHROPIC_MODEL").unwrap_or_else(|_| "glm-4.6".into());
    if key.is_empty() {
        return None;
    }
    Some((base, key, model))
...
fn openai_conf() -> Option<(String, String, String)> {
    if std::env::var("ION_E2E_OPENAI").is_err() {
        return None;
    }
    ...
    let key = std::env::var("ION_OPENAI_API_KEY")
        .unwrap_or_else(|_| std::env::var("ION_API_KEY").unwrap_or_default());
    let model = std::env::var("ION_OPENAI_MODEL").unwrap_or_else(|_| "deepseek-v4-flash".into());
    if key.is_empty() {
        return None;
    }
    Some((base, key, model))
...
async fn drain_stream(mut stream: EventStream, label: &str) -> AssistantMessage {
    ...
    while let Some(event) = stream.recv().await {
        ...
        match event {
            ...
            StreamEvent::ThinkingEnd { content, .. } => {
                println!("[{label}] ThinkingEnd ({} chars)", content.len())
            }
            StreamEvent::ToolCallStart { .. } => println!("[{label}] ToolCallStart"),
            StreamEvent::ToolCallDelta { delta, .. } => {
                println!("[{label}] ToolCallDelta: {delta}")
            }
            ...
            StreamEvent::Done { message, .. } => {
                println!(
                    "[{label}] Done: stop_reason={:?}, text_len={}, thinking_len={}, tool_calls={}",
                    message.stop_reason,
    ...
    panic!(
        "[{label}] stream ended without Done event (had_start={had_start}, had_done={had_done})"
    );
}
...
async fn anthropic_basic_stream() {
    ...
    let stream = provider
        .stream(&model, &ctx, Some(&opts), None)
        .await
        .expect("stream 创建失败");
...
fn mistral_conf() -> Option<(String, String)> {
    if std::env::var("ION_E2E_MISTRAL").is_err() {
        return None;
    }
...
fn mistral_model_id() -> String {
    std::env::var("ION_MISTRAL_MODEL").unwrap_or_else(|_| "mistral-large-latest".into())
}
...
async fn mistral_basic_stream() {
    ...
    assert!(
        ...
        "stop_reason 应该是 Stop/Length，实际 {:?}",
        msg.stop_reason,
    );
    let text: String = msg
        .content
