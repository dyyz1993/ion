...
//!
//! 对齐 pi packages/ai/src/providers/openai-responses.ts
        ...
        tokio::spawn(async move {
            if let Some(c) = &cancel_clone {
                tokio::select! {
                    ...
                    _ = c.cancelled() => {
                        tracing::info!("[stream] openai-responses SSE read cancelled by abort");
                    }
                }
            } else {
                ...
                for block in &a.content {
                    match block {
                        ...
                        AssistantContentBlock::Thinking(th) => {
                            // Reasoning items are replayed as reasoning items
                            if let Some(ref sig) = th.thinking_signature {
                                // Encoded signature carries id; content text may be empty for encrypted reasoning
                                input.push(serde_json::json!({
                                    "type": "reasoning",
                                    "id": sig,
...
async fn parse_sse_stream(
    ...
    // We track partial state for reasoning/text/tool_call separately
