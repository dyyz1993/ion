...
| [docs/design/FILESYSTEM_SNAPSHOT.md](./docs/design/FILESYSTEM_SNAPSHOT.md) | Filesystem Snapshot — 文件系统快照与回滚：自建 content-addressable 存储 + 双轨变更检测(工具拦截+文件扫描) + 三 Phase 路线 (设计稿, Phase 1 待实现) |
...
| [docs/design/FILE_SNAPSHOT.md](./docs/design/FILE_SNAPSHOT.md) | File Snapshot：双路快照（工具级 before/after + 目录扫描兜底），100MB GC 封顶 (已实现 Phase 1-7) |
...
| 套件 | 数量 | 覆盖 |
|------|------|------|
| lib tests (核心逻辑) | 288 | Agent/Permission/Retry/CommandGuard/Session/SessionTree/GlobalMemory/Memory/Worker/MessageRetrieval/SessionJsonl/SessionIndex/ContextIndex/SoftDeleteCompact/FileSnapshot |
...
| **测试覆盖合计** | **582** | 全部通过 ✅（session_tree_ci 废弃不计入） |
