...
| [docs/design/VISION_AND_NOTIFY.md](./docs/design/VISION_AND_NOTIFY.md) | **视觉输入与异步委派通知** — prompt images 参数 + 能力位判定 + 运行中 prompt 分派 + 完成通知回传（已验证） |
...
| `skills/` | **Agent Skills**（开放标准 SKILL.md 布局）：code-review / scheduler / goal-parsing，外部 agent 可 `npx skills add dyyz1993/ion` 一键安装；ION 自身放 `~/.ion/agent/skills/`（全局）或 `<project>/.ion/skills/`（项目级）；浏览门面 [dyyz1993.github.io/ion](https://dyyz1993.github.io/ion/)（`scripts/build_pages.py` + `.github/workflows/pages.yml` 自动部署） |
...
> **2026-09-16 修复批 7（统一审批总线——三来源收敛 + 外部桥接口，7 智能体两波）**：审批三家族（host UI Ask / file-snapshot review / remote verb）收敛为**内核统一审批总线**（[APPROVAL_BUS.md](./docs/design/APPROVAL_BUS.md)）：①ApprovalBus 内存统一表（ApprovalEntry{id,kind,summary,payload,nativeRequestId}，dedupe 幂等 + 30s 墓碑防镜像迟到重注册）②三来源接入——worker Ask 托管闭环（**J8 缺口根治**：runtime worker 模式 stdout 上报 + ask_respond 双臂 + approval_respond 路由回 worker 放行工具）/ verb 双登记 / file-snapshot 快照登记③单一路由 `execute_unified_approval`（host oneshot / worker ask_respond / verb_review / review_approve_all 四分支，RPC `approvals_pending`+`approval_respond`，id 兼容 apr_/native 前缀）④出口三件套（Push: ApprovalRequest/Resolved/Removed 事件 + snapshot approvals 字段；Pull: approvals_pending；Respond: approval_respond）——**外部桥接方（webui/IM/手机推送）两步接入**：subscribe + approval_respond⑤审批泵全来源 policy（per-kind map 兼容旧串，冷却键升 (worker,kind)，by=pump 归因）⑥旧 API 全兼容。CI 三合一 70 case。测试：lib 1192/0 · bin 81/0 · approval_bus_ci 70/0 · subscribe 26 · sandbox_policy 27 · rpc_schema_subscribe 19/0。⚠️ 附带发现并修：serve pump extension_event 死分支（worker 审批事件从未进 EventBus）；SessionRouter 退出丢弃排队 Subscribe（极端负载可复现，待后续）。🔴 本批事故：失控的巡检 cron 智能体三轮破坏主仓源码（03:29/04:43/04:53，删 8817 行重写 12 文件）——集成智能体从会话 diff 取证重建（警告集/测试数双重对齐验证），巡检 cron automation-21342b03 已暂停（701 轮，恢复需人类确认 + 加只读硬约束）。

> **2026-09-16 修复批 6（e2e 挖出的双 P0 根治，mock SSE 确定性复现路线首战）**：①**abort 后 run 永久挂起**——根因：`Agent::stop()` 的取消信号错用 pause watch channel 且全代码库无 resume 调用者（pause=true 永久残留，下一轮 run 卡在 check_pause 轮询，无 LLM 请求无 agent_end，后续 prompt 全被 steer 吞）；修复：abort 路径 send(false) + run 边界统一复位 pause（分层防御）；abort_hang_harness（脚本化本地 SSE 服务器走真实 HTTP 路径——FauxProvider 测不出的盲区由此补上）+ 真实 e2e T5 复验：abort 后同 PID worker 直接续答，不再需要 kill 重建 ②**prompt 撞 worker 重建窗口被吞**——根因：4dd79da（2026-08-10 parking_lot 重构）回归，handle_manager_command_write 默认分支把 session_id 当 worker_id 传 send_async（"worker not found: sess_xxx"，响应实为 success:false 但 `ion rpc` 不按响应置退出码）；修复：ensure_live_worker_for_session 三段决策（live 直转/有界等 AUTO-RECOVERY 替补 ≤1.5s/do_create_session 补拉后按 worker_id 投递）+ socket 层 live-first find（顺带修 subscribe 订到死记录）+ creating 集合泄漏修复；prompt_rebuild_window_harness 7 点位采样（kill 后 0/100/500/1000ms × respawn off/on）修前 4/4 永久吞没→修后 7/7 投递。测试：lib 1142/0 · bin 74/0 · abort_hang 1/1 · rebuild_window 7/7 · e2e 5/5（单独复验全绿；背靠背链跑有真实 LLM 时序抖动，J7 的重发兜底保留）。⚠️ 行为变更：Dead 会话的 create_session 从假复用变真补拉（ensure 语义）；abort 现在清 pause。

> **2026-09-15 修复批 5（清尾批：安全 P2+容错杠杆+沙盒池 P2+webui 同步+e2e）**：①SSH TOFU pin（host_key_fingerprint 指纹比对 fail-closed + 未配置时 SshHostKeyFirstSeen 事件；含非 22 端口 [host]:port 缺口）②host socket 重绑竞态（bind 仲裁+pid 带 hostId 身份+serve stop 真停——**顺带修掉 master 两个既有 bug：serve stop 假成功+盲删活 host 文件、stdin 阻塞读不可取消**，k1_smoke 19/19）③HTML 导出 XSS 三洞修复（fork-origin banner 未转义等，负向测试锁）④file_snapshot 排除 .ion 内部文件（scanner+approval 双层）⑤对账重建条目补真实统计 ⑥**同 sid 原地重建**（三档决策：本机文件/回流副本/摘要接力；failover 换节点自动降级保真度下限；harness 验全历史预加载+队列回放，CRASH_RECOVERY.md §9）⑦SANDBOX_POOL Phase 2 收官（approval_policy 档案+运行时覆盖 sandbox_policy RPC+host 审批泵治无人值守审批停摆+notes 环境层注入，sandbox_policy_ci 27/27）⑧ion-webui 协议同步（UiConnection 同源应答+hello hostId 重启检测+stale_route 立即重订+snapshot 帧，ion-webui 仓 39b053a，protocol_ci 11/0）⑨**E2E 真实 case 5 项全过**（hostId pin/subscribe 协议全链/队列落盘 kill -9/原地重建/abort_bash——真实 LLM glm-5.2，~45 轮）。测试：lib 1140/0 · bin 74/0 · export_ci 54/54 · k1_smoke 19/19 · sandbox_policy 27/27。🔴e2e 挖出 2 个产品 bug 待修（下一批 P0 候选）：**prompt 撞 worker 重建窗口被吞**（返回 success 但新 worker 永不跑）；**abort 后同 worker 下一轮 run 永久挂起**（agent_start 后无 LLM 事件，疑 cancellation token 残留，≥300s 不恢复，真实环境复现 2 次）。

> **2026-09-15 修复批 4（P0×2 + 容错遗留 P1/P2 全清）**：①abort_bash 改 tokio lock().await（治 blocking_lock 在 runtime 内 panic 杀 worker——G5 实测一发即死，恢复 exec schema 动态验证）②provider_bridge 分层超时（首块 120s/chunk 间 idle 对齐 ION_LLM_IDLE_TIMEOUT_MS，超时发 llm_cancel 停上游；REMOTE_WORKER 注入 600s 防重推理误断）③**steer/followUp 队列落盘**（custom/queued_input + 消费标记 + 重建回放三闸幂等——治 kill -9 排队消息蒸发黑洞，harness 验"新轮真实作答"非队列存在，见 CRASH_RECOVERY.md §8）④**索引逆向对账**（serve 启动 reconcile_missing：文件名差异集+header-only 重建，尊重墓碑，零差异零写入幂等——治索引落后会话成幽灵）⑤空补全视为协议错误重试（治 zai 幻影 turn 烧预算，防误伤纯 tool_call/thinking-only）⑥永久错误自动降级 tier_models（401/402/配额类→tier_fallback_candidates 严格语义（默认模型不泄漏进候选池）+ModelFallback 事件+model_change 落盘，runtime.auto_fallback_tier 默认开）⑦AUTO-RECOVERY 重派沙盒故障转移（池成员死→pick_healthy 换节点，全死回落+WARN，auto_recovered 事件带 failover 三态）⑧file_snapshot step 追加单调 seq（治同毫秒排序不稳，旧格式 seq=0 精确退化旧行为）。测试：lib 1105/0 · bin 69/0 · ion-provider 152/0 · protocol 39/0 · 新 harness 14/0（空补全 5+降级 3+队列 5+对账 1）· 特征 8/8 · CI：subscribe 26/session 45/host_read 20/line_limit 12/rpc_event_push 18。⚠️ 行为变更：auto_recovered 事件推迟到 probe_all 之后（池探测通常 <5s）；降级每档一次防翻转（fallback_tried 不随 run 重置）。

> **2026-09-15 修复批 3（schema 化挖出 5 bug + P0 adopt 收尾）**：①session_remove 改用 paths::sessions_dir()（治 ION_SESSION_DIR 下 removed_files 恒 0）②FileIndex 识别简形 message（治 fast_turns 0 turns 不回落，选索引层修因检索层早已支持简形）③review_pending 双路径形状统一（status 引号 bug + summary 恒四字段，收敛 schema）④FileSnapshot baseline 每会话 once-guard（治 call_tool 直调写入被吸收绕过审批）⑤bridge manager 等待分档超时 30/70/320s（治无 host 时挂死，ION_BRIDGE_TIMEOUT_MS 可调）⑥行长 16MiB 上限三处读点（P0.1 adopt，tests/line_limit_ci.sh 12/12）⑦错误响应脱敏+500 截断（P0.2，5 个构造器统一接线）⑧hello 带 hostId + ION_EXPECT_HOST_ID pin（P0.3，内存态不落盘）⑨exec 域 schema 43 命令补缺——**全量对账：对外命令 176 个，schema 覆盖 172，剩余为内部传输 outOfScope**。测试：lib 1083/0 · bin 69/0 · protocol crate 39/0 · 特征 8/8（🔴修了测试自身的 SUN_LEN 路径越界：长前缀+纳秒随机超 104 字节）· 五域 schema 测试全绿 · CI：session 45/host_read 20/subscribe_protocol 26/rpc_event_push 18/line_limit 12。🔴新发现未修：abort_bash 的 blocking_lock() 在 tokio 内 panic 杀死 worker（下一批 P0）；provider_bridge llm_request 无超时。
> 
> **2026-09-15 协议对标批次（阶段 B）**：对标 pi 上游新版（main@f9bcd351d，11 包）落地三件套——①P1 深读 pi protocol 包产出 [PI_PROTOCOL_ALIGNMENT.md](./docs/design/PI_PROTOCOL_ALIGNMENT.md)（3 adopt：JSONL 行长 16MiB 上限/错误脱敏截断/hello hostId pin；2 avoid：CBOR 换 JSONL、unknown-field 拒绝——均有论证）②**ion-protocol 信封 crate**（deps 仅 serde，26 单测；特征测试 6/6 双验等价；host 响应统一紧凑序列化=唯一行为变更；三处消费方收口，治手拼 JSON 分裂）③**RPC Schema 契约化**：`schemas/rpc/{session,worker,subscribe,config}/` 123 命令 + 28 事件帧 schema（draft 2020-12）+ 四域一致性测试（真实响应过 jsonschema 校验）+ 生成器 `scripts/gen_rpc_config_schemas.py`。测试：lib 1071/0 · bin 68/0 · crate 26/0 · 特征 6/6 · schema 测试 33/0 · subscribe_protocol_ci 20/20 · rpc_event_push_ci 18/18 · rpc_schema_session_ci 38/38 · rpc_schema_subscribe_ci 30/30。🔴 schema 化挖出 5 个真 bug 未修（转后续修复批）：session_remove 不吃 ION_SESSION_DIR、fast_turns 简形 0 turns 不回落、review_pending 双路径形状分裂、FileSnapshot baseline 每 prompt 重建（call_tool 写入被吸收）、bridge manager 转发无超时。遗留：执行域命令（prompt/steer/abort/follow_up）schema 未认领。
...
> - **分叉树闭环**：`fork {turnId,name?}`（从该 turn 尾 entry 落 leaf_pointer+label，原来是一行存根）+ `get_branches`（分支富列表：label/消息数/brief/当前标记 + main 虚拟主干）+ `checkout_branch {name}`（make_checkout 落盘，main 特判指主干最新链尾）——之后 get_messages 按 leaf 路径过滤只返回该分支；⚠️ worker 主循环 match 臂内错误路径禁止 `return`（会杀死 worker 进程，用闭包收 Result 再统一 output，2026-08-24 实测教训）（`tests/branch_tree_ci.sh` 13/13 含 D3 磁盘回落断言；前端⑂分叉预备态+分支树侧栏见 ion-webui）
> - **Host 协议四补全 + 元数据写保护（2026-08-25，commit de08097..9429004）**：get_session_messages/list_session_turns 并入 `--session` 直读拦截（修 CLI 路由 Unknown command）；`get_messages` 新增 `from=head` 顶部直跳；`list_inputs`/`get_turn_detail` host 直读（40ms 零拉起）；**subscribe 改 session 级绑定**（worker 未拉起挂起等待 60s 自动接上 + 死亡自动重接发 `resubscribed`）；SessionIndex upsert 写保护 + 复活 merge_existing_meta（含 project 归属，治消息落错目录）；save_worker_session 重放指纹防护（REPLAY DETECTED）；write 相对路径 resolve；file-snapshot session 归属 + 审批基线会话过滤 + Python 缓存忽略。新 RPC `turn_file_diff {turnId,path,base=before|prev|disk}`。测试：`tests/host_read_ci.sh` 20/20
...
>
> - **稳定性日（2026-08-28）**：worker 主循环补新命令名别名（get_session_messages 三路径全覆盖，治刷新空白）+ create_session 幂等守卫（31ms 双开竞态）+ 复活 cwd 兜底校验（spawn ENOENT）+ 循环检测累计口径（≥8 次非连续提醒）+ 会话存储大扫除（1889 孤儿归档）。全量验收：lib 984/0 · branch_tree_ci 13/13 · webui T1-T20 20/20 + 视觉 16 态 · 真实 LLM 冒烟 ✓（08-27 另有：MCP worker RPC 名对齐 get_mcp_servers）
> - **视觉输入 + 异步委派通知 + Follow-up 双按钮（2026-08-28，commit 821ba5b..09f6e4c）**：prompt RPC 接受 `images:[{data(base64),mimeType}]`（serde alias mimeType），ContentBlock::Image 拼 user 消息；修 3 个"视觉从未通车"根因（registry 解析器硬编码 input=["text"] 忽略能力位——权威源 ~/.pi/agent/models.json；set_model 不调 agent.set_model 运行时永不生效；/api/workers model 是出生值）；agent.run_with_images + 忙时 steer/followUp/select 臂全透传 images；运行中 prompt(followUp/steer) 走 follow_up_tx 通道活递（原 select 臂不识别 prompt 落 busy 兜底 + 外部缓存队列只在 run 结束后 drain=黑洞）；异步委派完成通知（record.notify_parent=有父&&wait==false，泵 agent_end 读 latest_output→session→worker id 解析→prompt(followUp) 注入父，ION_CHILD_NOTIFY=0 关）。测试：`tests/vision_notify_ci.sh` 12/12（视觉判别/通知回灌/followUp 消费开新轮）+ ion-console `tests/ui.test.mjs` T17/T18/T19。设计文档：`docs/design/VISION_AND_NOTIFY.md`
> - **稳定性三补（2026-08-28，commit 2d24f2f/8c73be3）**：send_async 等 5 处 stdin take/put 写互斥竞态（并发 RPC 撞车静默丢命令→15s 黑洞，修复=acquire_stdin 有界重试）；仅工作类命令（prompt/steer/abort）标 Busy（原一切 RPC 标 Busy 被 UI 30s 轮询刷回，状态点永久忙碌）；ION_LLM_MAX_RETRIES 可调（8 provider 均有 180-300s 总超时+SSE 空闲超时，无无限挂）
