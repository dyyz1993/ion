...
| [docs/design/PROVIDER_PROTOCOLS_TODO.md](./docs/design/PROVIDER_PROTOCOLS_TODO.md) | 缺失 Provider 协议规划：Mistral+Bedrock 已实现，剩 Azure/Codex/Vertex 3 个协议待补齐 (开发中) |
...
- **Extension Host API（ctx.fs 统一文件访问，对齐 pi ExtensionContext.fs）**:
  ...
  - `ExtensionDataDirs` struct（session/cwd/project/global 4 级数据目录）+ `ExtensionRegistry.data_dirs(name)` getter
  - `ExtensionRegistry.fs` / `.storage` 字段 + `with_filesystem()` / `with_storage()` 访问器（向后兼容，Option 字段）
  ...
  - **验证**: 18 单元（15 fs + 3 data_dirs）+ 23 CI 测试全过 ✅
...
- **设计文档**：[docs/design/GOAL_SUPERVISOR.md](./docs/design/GOAL_SUPERVISOR.md)（含 §13 深化章节）+ [GOAL_SUPERVISOR_B1_TASK.md](./docs/design/GOAL_SUPERVISOR_B1_TASK.md)
- **深化（§13）**：
  - **进展分析**：每次 gate RetryWith 前自动分析趋势（Converging/Oscillating/Stagnant/Drifting）+ 建议注入消息
  - **目标校正**：`goal_refine` tool（增量改检测项/objective，保留 iteration_count/cost）
  - **偏离监控**：`on_tool_execution_end` 追踪工具行为（recent_tools 滑动窗口）+ 工具级 drift 检测
  - **诊断 agent**：`goal_diagnose` tool → spawn goal-diagnostician（3 维度诊断：检测项质量/agent 能力/目标可行性）
  - **3 个 tool**：goal_set（覆盖+B2 自动生成）+ goal_refine（增量调整）+ goal_diagnose（专家诊断）
- **验证**: 35 单元测试 + goal_supervisor_ci 26 checks + goal_supervisor_e2e 8 checks + 真实 GLM-5.2 验证 ✅

### 🧬 Goal Evolver（日志分析进化系统，B3 已完成）

- **分析核心**（`src/goal_evolver.rs`）：读 iterations.jsonl + final-report.json → 3 维度分析 → Issue 计划
  - Q1 死循环：repetitive+abandoned、stuck check、thrashing（不同错误反复）
  - Q2 模型：generate_checks 弱模型、analyze_failure 从未被采纳
  - Q3 上下文：test_results_included=false、git_diff_lines=0
- **10 个 fixture 场景**（`tests/fixtures/goal-runs/`）：覆盖健康/死循环/模型/上下文/边界 5 维度
- **RPC**：`goal_evolver_run_once`（data_dir + dry_run，dry_run 默认 true 不误提交）
- **Agent 定义**：`examples/agents/goal-evolver.md`（定时跑 → 分析 → 提主仓库 Issue，不直接改代码）
- **双模式触发**：生产（MonitorExtension 定时 24h/10 goal）+ 手动（fixture dry_run 秒级验证）
...
**P9 - WASM 扩展生态 + 上下文压缩 (v0.2.0):** ✅ 已完成
- **WASM host functions 补全**: 17 → 27 个（+9 agent 状态：get_token_count/get_messages/get_state/steer/llm_call/get_worker_status/inject_follow_up/compact_now/create_worktree；+3 文件系统：write_file/path_exists/glob）
- **WASI stubs**: std-compiled WASM 支持（environ_get/fd_write/proc_exit/random_get/clock_time_get）
- **3 个 WASM 扩展**（用户可 DIY 安装）：
  - `rules-engine.wasm` — 项目规则注入（✅ 端到端验证：规则 → system prompt → LLM 遵守）
  - `file-time-guard.wasm` — 过期文件写入拦截（✅ 加载验证通过）
  - `session-supervisor.wasm` — TODO/FIXME 残留检测 + steer（✅ 加载验证通过）
- **Context Reclaimer**（内置扩展）：优先级 token 回收（thinking 剥离 + bash>grep>read 优先级 + 热度窗口 30% + stale read 检测），实测 73.9% token 节省
- **Memory V0.3**: entities + relations 知识图谱（12 方法 + 12 测试）
- **Watchdog 安全升级**: 心跳监控 + 自动回滚（3 场景验证：编译失败/启动崩溃/进程卡死）
- **evolve_auto.sh**: 一条命令完成 A→B 闭环（改代码 → 守门 → PR/commit → watchdog 升级 → 清理）
- **SSE 空闲超时**: 120 秒无 chunk 自动断连（修复 zai proxy reasoning 阶段断连）
- 777 tests passed, 0 failed
