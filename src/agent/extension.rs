...
pub trait Extension: Send + Sync {
    ...
    async fn on_session_before_compact(&self, _msgs: &mut Vec<Message>) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_session_before_switch(&self, _ctx: &SessionSwitchContext) -> AgentResult<()> {
        Ok(())
    }
    /// Fired before a fork-from-leaf (cross-file). Return Err to veto.
    async fn on_session_before_fork(&self, _ctx: &SessionSwitchContext) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_input(&self, _ctx: &mut InputContext) -> AgentResult<()> {
        Ok(())
    }
    ...
    // ── Context / Provider (3) ──
    async fn on_context(&self, _messages: &mut Vec<Message>) -> AgentResult<()> {
        Ok(())
    }
    async fn before_provider_request(&self, _ctx: &ProviderRequestContext) -> AgentResult<()> {
        Ok(())
    }
    async fn after_provider_response(&self, _ctx: &ProviderResponseContext) -> AgentResult<()> {
        Ok(())
    }
    /// 自动重试开始（LLM 调用失败后退避重试）。前端可显示"重试中 (N/M)..."
    async fn on_auto_retry_start(&self, _attempt: u32, _max_retries: u32) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_message_delta(&self, _delta: &str, _role: &str) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_thinking_delta(&self, _delta: &str) -> AgentResult<()> {
        Ok(())
    }
    /// Called when thinking content is complete.
    async fn on_thinking_end(&self, _content: &str) -> AgentResult<()> {
        Ok(())
    }
    /// Called for tool call deltas during streaming (partial tool name/args).
    async fn on_tool_call_delta(&self, _delta: &str, _name: &str) -> AgentResult<()> {
        Ok(())
    }
    /// Called when a text block ends (provider's TextEnd event).
    async fn on_text_end(&self, _content: &str) -> AgentResult<()> {
        Ok(())
    }
    /// Called when a tool call completes (provider's ToolCallEnd event).
    async fn on_tool_call_end(&self, _tool_call: &ToolCall) -> AgentResult<()> {
        Ok(())
    }

    // ── Tool execution (5) ──
    async fn on_tool_execution_start(&self, _ctx: &ToolExecutionContext) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_tool_execution_end(&self, _ctx: &ToolExecutionContext) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_model_select(&self, _ctx: &mut ModelSelectContext) -> AgentResult<()> {
        Ok(())
    }
    async fn on_thinking_level_select(&self, _level: &str, _old: Option<&str>) -> AgentResult<()> {
        Ok(())
    }

    // ── Entries ──
    /// Called when entries are deleted or summarized.
    async fn on_entries_invalidated(&self, _entry_ids: &[String]) -> AgentResult<()> {
        Ok(())
    }

    // ── Session navigation (stubs - 已在上面定义 on_session_before_switch/fork) ──
    /// Called before tree navigation. Can customize summary.
    async fn on_session_before_tree(&self, _target: &str) -> AgentResult<()> {
        Ok(())
    }
    /// Called after tree navigation.
    async fn on_session_tree(&self, _leaf_id: &str) -> AgentResult<()> {
        Ok(())
    }
    ...
    async fn on_user_join(&self, _worker_id: &str) -> AgentResult<()> {
        Ok(())
    }

    /// 有 Worker 停止使用此单例时调用（引用计数 -1）。
    /// 某个 Worker 崩溃/退出 → 触发此钩子，但单例不关。
    async fn on_user_leave(&self, _worker_id: &str) -> AgentResult<()> {
        Ok(())
    }

    /// 最后一个用户离开时调用（引用计数 == 0）。
    /// 单例可在此决定是否关闭自己。
    async fn on_last_user_gone(&self) -> AgentResult<()> {
        Ok(())
    }
...
impl RuntimeFileSystem {
    ...
    pub fn default_allowed_roots(project_root: &std::path::Path) -> Vec<std::path::PathBuf> {
        vec![project_root.to_path_buf(), crate::paths::root()]
    }
...
impl ExtensionRegistry {
    ...
    pub async fn on_message_start(&self, role: &str, content: &str) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_message_start(role, content).await?;
        }
        Ok(())
    }
    pub async fn on_message_delta(&self, delta: &str, role: &str) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_message_delta(delta, role).await?;
        }
        Ok(())
    }
    pub async fn on_message_end(
        &self,
        role: &str,
        content: &str,
        usage: &Usage,
    ...
    pub async fn on_before_tool_execute(
        ...
        for ext in &self.extensions {
            ext.on_before_tool_execute(tool_name, args, messages)
                .await?;
        }
        Ok(())
    }
    pub async fn on_tool_execution_start(&self, ctx: &ToolExecutionContext) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_tool_execution_start(ctx).await?;
        }
        Ok(())
    }
    pub async fn on_tool_execution_update(
        ...
        for ext in &self.extensions {
            ext.on_tool_execution_update(ctx, partial).await?;
        }
        Ok(())
    }
    pub async fn on_tool_execution_end(&self, ctx: &ToolExecutionContext) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_tool_execution_end(ctx).await?;
        }
        Ok(())
    }
    ...
    pub async fn after_tool_call(
        ...
        for ext in &self.extensions {
            ext.after_tool_call(call, result).await?;
        }
        Ok(())
    }
    pub async fn on_model_select(&self, ctx: &mut ModelSelectContext) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_model_select(ctx).await?;
        }
        Ok(())
    }
    pub async fn on_thinking_level_select(
        &self,
        level: &str,
        old: Option<&str>,
    ) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_thinking_level_select(level, old).await?;
        }
        Ok(())
    }
    pub async fn on_system_prompt(&self, prompt: &mut String) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_system_prompt(prompt).await?;
        }
        Ok(())
    }
    ...
    pub async fn on_entries_invalidated(&self, entry_ids: &[String]) -> AgentResult<()> {
        for ext in &self.extensions {
            ext.on_entries_invalidated(entry_ids).await?;
        }
        Ok(())
    }
    ...
    pub async fn extension_rpc(
        ...
        for ext in &self.extensions {
            ...
            match result {
                Ok(v) => return Ok(v),
                Err(AgentError::Tool(ref msg)) if msg == "extension rpc method not found" => {
                    continue;
                }
                Err(e) => return Err(e),
            }
        ...
        Err(AgentError::Tool(format!(
            "extension '{extension_name}' not found or method '{method}' not implemented"
        )))
    }
...
mod fs_tests {
    ...
    impl crate::runtime::Runtime for TestRuntime {
        ...
        async fn file_info(&self, _p: &str) -> Result<Vec<crate::runtime::FileEntry>, String> {
            Err("no".into())
        }
    ...
    async fn glob_double_star_recursive() {
        ...
        assert!(
            matches.iter().any(|m| m.contains("top.rs")),
            "matches: {matches:?}"
        );
        assert!(
            matches.iter().any(|m| m.contains("deep.rs")),
            "matches: {matches:?}"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
...
mod data_dirs_tests {
    ...
    fn data_dirs_returns_four_levels_with_ext_name() {
        ...
        assert!(
            s.contains("sess_abc"),
            "session should contain session_id: {s}"
        );
    }
    ...
    fn data_dirs_different_ext_names_isolate() {
        ...
        assert_ne!(
            a.global, b.global,
            "different exts must have different dirs"
        );
